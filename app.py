...
FINNGEN_PATH = "/Users/sarahmughal/Desktop/epi genetics/GWAS-Explorer/finngen_R12_T2D_small.parquet"


@st.cache_data(show_spinner=False)
def load_finngen(path: str) -> pd.DataFrame:
    """Load FinnGen R12 summary stats from local .gz file."""
...
def parse_upload(uploaded_file) -> pd.DataFrame:
    ...
    for c in raw.columns:
        if c in ("P", "P_VALUE", "PVALUE", "P-VALUE"):
            col_map[c] = "PVAL"
        if c in ("CHROMOSOME", "CHROM", "#CHROM"):
            col_map[c] = "CHR"
        if c in ("POSITION", "BP"):
            col_map[c] = "POS"
        if c in ("EFFECT", "B", "BETA"):
            col_map[c] = "BETA"
        if c in ("RS_ID", "SNP", "ID", "MARKER", "RSIDS"):
            col_map[c] = "RSID"
        if c in ("SEBETA", "STANDARD_ERROR"):
            col_map[c] = "SE"
        if c in ("AF_ALT", "EAF", "FREQ"):
            col_map[c] = "MAF"
        if c in ("NEAREST_GENES",):
            col_map[c] = "GENE"
    ...
    raw["CHR"] = pd.to_numeric(
        raw["CHR"].astype(str).str.replace("chr", "", case=False), errors="coerce"
    )
...
def qq_plot(pvals, lam):
    ...
    sample_idx = np.random.choice(
        np.where(~extreme_mask)[0], size=min(5000, (~extreme_mask).sum()), replace=False
    )
    ...
    fig.add_trace(
        go.Scattergl(
            ...
            marker=dict(
                ...
                colorbar=dict(
                    title=dict(
                        text="r² (proxy)", font=dict(family="IBM Plex Mono", size=11)
                    ),
                    thickness=12,
...
def main():
    ...
    with st.sidebar:
        ...
        data_source = st.radio(
            "Source", ["FinnGen R12 T2D (default)", "Upload your own file"], index=0
        )
        uploaded_file = None
    ...
    if uploaded_file is not None:
        ...
        if not os.path.exists(FINNGEN_PATH):
            st.error(
                f"FinnGen file not found at:\n`{FINNGEN_PATH}`\n\nPlease check the path or switch to 'Upload your own file'."
            )
            st.stop()
        with st.spinner(
            "Loading FinnGen R12 T2D summary statistics — this may take a minute on first load..."
        ):
    ...
    with col_m:
        st.plotly_chart(
            manhattan_plot(df, pval_thresh), use_container_width=True, key="manhattan"
        )
    with col_q:
        st.plotly_chart(
            qq_plot(df["PVAL"].values, lam), use_container_width=True, key="qq"
        )

    st.divider()
    st.markdown('<p class="section-header">Top Hits</p>', unsafe_allow_html=True)
    show_cols = (
        ["RSID", "GENE", "CHR", "POS", "PVAL", "BETA", "SE", "MAF", "INFO"]
        if "GENE" in df.columns
        else ["RSID", "CHR", "POS", "PVAL", "BETA", "SE", "MAF", "INFO"]
    )
    top_hits = (
        df[df["PVAL"] < pval_thresh]
        .sort_values("PVAL")
        .head(200)[[c for c in show_cols if c in df.columns]]
        .copy()
    )
    ...
    if top_hits.empty:
        ...
        for _, row in sig_df.head(30).iterrows():
            gene = (
                f"  [{row['GENE']}]"
                if "GENE" in sig_df.columns and pd.notna(row.get("GENE"))
                else ""
            )
            locus_options.append(
                f"{row.RSID}{gene}  (Chr{row.CHR}:{row.POS:,}  P={float(row.PVAL):.2e})"
            )
    ...
    st.divider()
    st.markdown(
        "<p style='color:#484f58; font-family: IBM Plex Mono, monospace; font-size:0.72rem; text-align:center'>GWAS Explorer · Epi Final Project · FinnGen R12 T2D · ~500k Finnish participants</p>",
        unsafe_allow_html=True,
    )
